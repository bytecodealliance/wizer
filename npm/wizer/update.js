...
for (const [packageName, info] of Object.entries(packages)) {
    ...
    const downloadResp = await fetch(browser_download_url);
    if (!downloadResp.ok) {
        console.error(`Response from ${browser_download_url} was not ok`, archive);
        console.error(await response.text());
        process.exit(1);
    }
    let downloadBytes = await downloadResp.arrayBuffer();
    ...
    if (info.releaseAsset.endsWith('.xz')) {
        // Decompress and write to local file on disk, then clean up the ephemeral file
        downloadBytes = await xz.decompress(new Uint8Array(downloadBytes));
    }
